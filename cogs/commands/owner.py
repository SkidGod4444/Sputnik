...
#https://cdn.discordapp.com/avatars/974984890959425566/7fedaa654af7ec62b211033852e048d0.webp?size=2048
  ...
  async def blacklist(self, ctx):
    if ctx.invoked_subcommand is None:
      with open("blacklist.json") as file:
                ...
                embed = discord.Embed(
                title=f"{len(blacklist['ids'])} blacklisted!",
    ...
    try:
      with open('blacklist.json', 'r') as bl:
        ...
        if str(member.id) in blacklist["ids"]:
          ...
          embed = discord.Embed(title="Blacklisted", description=f"<:tixk:1053178188613820468> Successfully Blacklisted {member.name}", color=0xdbdbdb)
          with open("blacklist.json") as file:
              blacklist = json.load(file)
              embed.set_footer(
  ...
  async def blacklist_remove(self, ctx, member: discord.Member = None):
    try:
      ...
      embed = discord.Embed(
                title="Unblacklisted",
                description=f"<:tixk:1053178188613820468> | **{member.name}** has been successfully removed from my blacklist",
                color=0xdbdbdb
            )
      with open("blacklist.json") as file:
        blacklist = json.load(file)
