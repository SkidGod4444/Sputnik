...
class ready(commands.Cog):
    ...
    async def on_shard_ready(self, shard_id):
      lol = self.client.get_channel(1065300503547879545)
      embed = discord.Embed(description=f"{shard_id} is ready!")
    ...
    async def on_shard_connect(self, shard_id):
      lol = self.client.get_channel(1065300503547879545)
      embed = discord.Embed(description=f"{shard_id} is connected!")
      await lol.send(embed=embed)
    ...
    async def on_shard_disconnect(self, shard_id):
      lol = self.client.get_channel(1065300503547879545)
      embed = discord.Embed(description=f"{shard_id} is disconnected!")
      await lol.send(embed=embed)
    ...
    async def on_shard_resume(self, shard_id):
      lol = self.client.get_channel(1065300503547879545)
      embed = discord.Embed(description=f"{shard_id} is resumed!")
      await lol.send(embed=embed)
